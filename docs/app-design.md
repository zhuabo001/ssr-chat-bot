...
构建一个可部署到 Vercel Hobby 的服务端渲染 Web 应用。应用的第一核心体验是多用户登录后的 AI 流式对话，同时支持 Markdown 文档上传、自定义 OpenAI-compatible 模型服务端点配置、会话侧边栏切换，以及高度复刻 Codex 桌面端的工作台布局和视觉风格。

当前仓库基本为空，因此本文档按从零搭建方案设计。

## 1.1 已确认决策

1. 账号体系: 第一版需要支持多用户账号体系。
...
1. 用户可以注册/登录，并只能访问自己的会话、消息、文档和模型配置。
...
3. 可选缓存/限流: Upstash Redis。第一版如果只做低流量演示，可先用数据库计数或内存级保护，但多用户公开部署最终需要外部限流存储。
...
1. `v1-env-only`: 第一版只支持平台管理员配置一个或少量默认 OpenAI-compatible endpoint，用户不能保存自己的 API key。此路径可以完全使用 Vercel env。
...
1. 左侧侧边栏: 会话列表、搜索、新建会话、设置入口。
2. 中间主区域: 当前会话消息流、文档引用提示、输入框。
3. 右侧检查器: 当前会话上下文、上传文档、模型参数、运行状态。右侧检查器在小屏幕上改为抽屉。

### 4.2 主要页面

#### `/`

应用工作台。默认打开最近会话；若没有会话，创建一个临时空会话。

#### `/chat/:sessionId`

指定会话工作台。支持 SSR 加载会话元数据和历史消息，客户端接管后继续流式对话。

#### `/settings/models`

模型端点配置页。支持新增、编辑、测试连接、设置默认模型。

#### `/settings/profile`

用户偏好。第一版可以只保留主题、默认模型、语言策略、数据保留策略。语言默认跟随浏览器，可手动覆盖为中文或英文。
...
1. 前端发送 `sessionId`、用户消息、选定 `modelEndpointId`、附件引用。
2. 服务端校验 session 权限、模型配置、附件可访问性。
3. 服务端读取最近消息和选定 Markdown 文档内容。
4. 服务端构造 prompt，按 token budget 截断上下文。
5. 服务端调用模型 provider 并流式返回。
6. 前端边收边渲染。
7. 流结束后服务端持久化 assistant message 和 usage metadata。
...
3. 第一版如果选择 `v1-env-only`，模型设置页只能允许用户选择平台 endpoint 和调整非敏感参数。
...
认证相关路由由所选 auth provider 暴露。业务 API 的默认策略是 authenticated-only。
...
10. 多用户版本必须实现认证中间件，所有 server API 默认拒绝匿名请求，登录页和 OAuth callback 除外。
...
### Phase 1: 工程骨架、认证入口与 UI Shell

1. 初始化 Nuxt + TypeScript。
2. 接入 Tailwind 和基础 design tokens。
3. 建立 i18n dictionary，默认跟随浏览器语言。
4. 完成高度贴近 Codex 桌面端的 AppShell、Sidebar、ChatWorkspace、Composer 静态结构。
5. 增加登录态入口、用户菜单和受保护页面骨架。
6. 使用 mock data 完成会话切换。
...
1. 接入 Auth.js、Clerk 或 Supabase Auth。
2. 接入 Postgres provider。
3. 建表和 migration。
4. 实现 user scope 下的 session/message CRUD。
5. 侧边栏连接真实数据。
6. 所有业务 API 增加 authenticated-only guard。
